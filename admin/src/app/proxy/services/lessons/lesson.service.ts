...
import type { CreateUpdateLessonDto, GetListLessonRequestDto, LessonDto } from '../dtos/lessons/models';
...
export class LessonService {
  ...
  getList = (input: GetListLessonRequestDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<LessonDto>>({
      method: 'GET',
      url: '/api/app/lesson',
      params: { chapterId: input.chapterId, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount, filter: input.filter },
    },
    { apiName: this.apiName,...config });
