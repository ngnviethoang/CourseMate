...
namespace CourseMate.ServiceDefaults;

public static class ServiceReferenceExtensions
{
    /// <summary>
    ///     Adds an HTTP service reference. Configures a binding between the <typeparamref name="TClient" /> type and a named
    ///     <see cref="HttpClient" />
    ///     with a base address. The client name will be set to the type name of <typeparamref name="TClient" />.
    /// </summary>
    /// <typeparam name="TClient">
    ///     The type of the typed client. The type specified will be registered in the service collection as a transient
    ///     service. See
    ///     <see cref="Microsoft.Extensions.Http.ITypedHttpClientFactory{TClient}" /> for more details about authoring typed
    ///     clients.
    /// </typeparam>
    /// <param name="services">The <see cref="IServiceCollection" />.</param>
    /// <param name="baseAddress">
    ///     The value to assign to the <see cref="HttpClient.BaseAddress" /> property of the typed
    ///     client's injected <see cref="HttpClient" /> instance.
    /// </param>
    /// <returns></returns>
        ...
        where TClient : class
    {
        ...
        return services.AddHttpClient<TClient>(c => c.BaseAddress = new Uri(baseAddress));
    }

    /// <summary>
    ///     Adds an HTTP service reference. Configures a binding between the <typeparamref name="TClient" /> type and a named
    ///     <see cref="HttpClient" />
    ///     with a base address and health check. The client name will be set to the type name of
    ...
    ///     The name for the health check. Defaults to <c>"{typeof(TClient).Name}-health"</c> if not
    ///     provided.
    /// </param>
    /// <param name="failureStatus">
    ///     The <see cref="HealthStatus" /> that should be reported if the health check fails. Defaults
    ///     to <see cref="HealthStatus.Unhealthy" />.
    /// </param>
    /// <returns>An <see cref="IHttpClientBuilder" /> that can be used to configure the client.</returns>
    /// <exception cref="ArgumentException">
    ///     Thrown when <paramref name="baseAddress" /> or
    ///     <paramref name="healthRelativePath" /> are not valid URI values.
    /// </exception>
    ...
    /// <summary>
    ///     Adds a gRPC service reference. Configures a binding between the <typeparamref name="TClient" /> type and a named
    ///     <see cref="HttpClient" />
    ///     with an address. The client name will be set to the type name of <typeparamref name="TClient" />.
    /// </summary>
    /// <typeparam name="TClient">
    ///     The type of the gRPC client. The type specified will be registered in the service collection
    ///     as a transient service.
    /// </typeparam>
    /// <param name="services">The <see cref="IServiceCollection" />.</param>
    /// <param name="address">
    ///     The value to assign to the <see cref="Grpc.Net.ClientFactory.GrpcClientFactoryOptions.Address" /> property of the
    ///     typed gRPC client's injected
        ...
        where TClient : class
    {
        ...
        IHttpClientBuilder builder = services.AddGrpcClient<TClient>(o => o.Address = new Uri(address));
    ...
    ///     Adds a gRPC service reference. Configures a binding between the <typeparamref name="TClient" /> type and a named
    ///     <see cref="HttpClient" />
    ///     with an address and gRPC-based health check. The client name will be set to the type name of
    ///     <typeparamref name="TClient" />.
    /// </summary>
    /// <remarks>
    ///     Note that the gRPC service must be configured to use gRPC health checks. See
    ///     https://learn.microsoft.com/aspnet/core/grpc/health-checks for more details.
    /// </remarks>
    /// <typeparam name="TClient">
    ///     The type of the gRPC client. The type specified will be registered in the service collection
    ///     as a transient service.
