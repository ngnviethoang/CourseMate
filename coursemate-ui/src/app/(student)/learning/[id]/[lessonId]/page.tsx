        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {content.segments.map((seg, i) => (
              <div key={i} className="group p-4 rounded-xl border bg-card hover:border-primary/30 transition-all cursor-default relative overflow-hidden">
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-xl border bg-card/50 overflow-hidden">
            {content.timestamps.map((ts, i) => (
              ...
              >
                <span className="text-xs font-mono font-bold text-blue-600 group-hover:scale-110 transition-transform">{ts.time}</span>
...
function ReadingView({ content }: { content: ReadingContent }) {
  const renderMarkdown = (md: string) => {
    return md.split('\n').map((line, i) => {
      if (line.startsWith('# ')) return <h1 key={i} className="text-3xl font-bold mt-8 mb-4 text-primary">{line.slice(2)}</h1>
      if (line.startsWith('## ')) return <h2 key={i} className="text-2xl font-bold mt-6 mb-3 border-b-2 border-primary/10 pb-1 inline-block">{line.slice(3)}</h2>
      if (line.startsWith('### ')) return <h3 key={i} className="text-xl font-semibold mt-4 mb-2 text-foreground/80">{line.slice(4)}</h3>
      if (line.startsWith('```')) return null
      if (line.startsWith('- ')) return <li key={i} className="text-base ml-6 list-disc mb-2 text-muted-foreground leading-relaxed">{line.slice(2)}</li>
      if (line.startsWith('> ')) return (
        <blockquote key={i} className="border-l-4 border-primary pl-6 py-4 text-lg text-foreground/80 italic my-6 bg-primary/5 rounded-r-xl">
          {line.slice(2)}
        </blockquote>
      )
      if (line.trim() === '') return <div key={i} className="h-4" />
      return <p key={i} className="text-base leading-relaxed text-muted-foreground mb-4">{line}</p>
    })
  }
    ...
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 h-full min-h-[600px] animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col gap-6">
        ...
        <div className="rounded-2xl border bg-card p-6 shadow-sm overflow-hidden">
          ...
          <div className="space-y-3">
            {content.test_cases.filter(t => !t.hidden).map((tc, i) => (
              <div key={i} className="group rounded-xl bg-muted/50 border p-3 font-mono text-xs relative overflow-hidden">
      ...
      <div className="flex flex-col gap-4">
        <div className="flex-1 rounded-2xl border bg-slate-900 shadow-2xl flex flex-col overflow-hidden ring-1 ring-white/10">
          <div className="px-4 py-2 border-b border-white/10 bg-white/5 flex items-center justify-between">
            ...
            <Badge variant="outline" className="text-[10px] text-white/40 border-white/10">Python 3.10</Badge>
          </div>
          <textarea
            value={userCode}
          ...
          <div className="p-4 border-t border-white/10 bg-white/5 flex gap-3">
            <Button size="sm" variant="ghost" className="text-white/60 hover:text-white hover:bg-white/10 border border-white/10">
              Run Code
            </Button>
            <Button size="sm" className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-500/20">
              Submit Solution
            </Button>
          </div>
        </div>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 py-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col items-center text-center space-y-2 mb-8">
        ...
        <Badge variant="outline" className="mt-2">{content.questions.length} Questions • 70% to Pass</Badge>
      </div>

      <div className="space-y-6">
        {content.questions.map((q, qi) => (
          <div key={qi} className={`p-6 rounded-2xl border bg-card shadow-sm space-y-4 transition-all ${submitted && answers[qi] === q.ans ? 'ring-2 ring-emerald-500/20 border-emerald-500/30' : ''}`}>
            ...
            <div className="grid grid-cols-1 gap-3">
              {q.options.map((opt, oi) => {
                ...
                return (
                  ...
                  >
                    <div className={`h-6 w-6 rounded-full border-2 flex items-center justify-center text-xs font-bold shrink-0 ${isSelected ? 'border-primary bg-primary text-white' : 'border-muted-foreground/30'}`}>
      ...
      <div className="pt-8 border-t">
        {!submitted ? (
          ...
          <div className="text-center space-y-6">
            <div className={`p-8 rounded-3xl border-4 ${pass ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950/20' : 'border-red-500 bg-red-50 dark:bg-red-950/20'}`}>
              ...
              <p className="font-semibold">{pass ? '🎉 Excellent! You PASSED.' : '💪 Keep practicing. You can do it!'}</p>
            </div>
            <Button variant="outline" size="lg" onClick={() => { setAnswers({}); setSubmitted(false) }}>
              Retry Quiz
            </Button>
          </div>
        )}
...
export default function StudentLearningPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      studentService.getLessonById(lessonId),
      studentService.getCourseById(id)
    ]).then(([l, course]) => {
      setLesson(l)
      ...
      if (stored) {
        try { setAiContent(JSON.parse(stored)) } catch (e) { console.error("Parse AI error:", e) }
      }
    ...
    return (
      <div className="flex h-[60vh] items-center justify-center">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-sm font-medium text-muted-foreground animate-pulse">Preparing your learning environment...</p>
        </div>
      </div>
    )
  }
  ...
  if (!lesson) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-xs">This lesson might have been moved or removed from the syllabus.</p>
      ...
      <div className="flex-1 overflow-y-auto pb-20">
        {aiContent || lesson.videoUrl || lesson.readingContent || lesson.starterCode ? (
          <div className="animate-in fade-in duration-700">
            {LESSON_TYPE_MAP[lesson.lessonType as any] === 'Video' && (aiContent ? 'segments' in aiContent && <VideoPlayer content={aiContent as VideoContent} /> : lesson.videoUrl && (
            ...
            {LESSON_TYPE_MAP[lesson.lessonType as any] === 'Reading' && (aiContent ? 'markdown_content' in aiContent && <ReadingView content={aiContent as ReadingContent} /> : lesson.readingContent && (
              <ReadingView content={{ title: lesson.title, markdown_content: lesson.readingContent }} />
            ))}
            {LESSON_TYPE_MAP[lesson.lessonType as any] === 'Coding' && (aiContent ? 'test_cases' in aiContent && <CodingExercise content={aiContent as CodingContent} /> : lesson.starterCode && (
              <CodingExercise content={{
                title: lesson.title,
                problem_statement: lesson.problemStatement || '',
                initial_code: lesson.starterCode || '',
                solution: '',
                test_cases: lesson.expectedOutput ? [{ input: '', output: lesson.expectedOutput, hidden: false }] : []
              }} />
            ))}
            {LESSON_TYPE_MAP[lesson.lessonType as any] === 'Quiz' && (aiContent ? 'questions' in aiContent && <QuizInteraction content={aiContent as QuizContent} /> : lesson.quizDescription && (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center space-y-6 bg-muted/20 border border-dashed rounded-3xl animate-in fade-in zoom-in duration-500">
            <div className="h-20 w-20 bg-background rounded-3xl shadow-sm border flex items-center justify-center text-muted-foreground rotate-3">
              {LESSON_TYPE_MAP[lesson.lessonType as any] === 'Video' ? <PlayCircle className="h-10 w-10" /> : <FileText className="h-10 w-10" />}
            </div>
            <div className="space-y-2">
              <h3 className="text-xl font-bold">Content Coming Soon</h3>
              <p className="text-muted-foreground max-w-sm">
                We're currently preparing the {(LESSON_TYPE_MAP[lesson.lessonType as any] || '').toLowerCase()} material for this lesson. Please check back later!
              </p>
            </div>
            <Button variant="secondary" onClick={() => router.push(`/courses/${id}`)}>Explore Syllabus</Button>
          </div>
        )}
      </div>
