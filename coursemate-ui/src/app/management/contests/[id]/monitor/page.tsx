...
export default function ContestMonitorPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 return (
 <div key={student.studentId} className={`rounded-2xl transition-all ${getStatusColor(student)} ${isSuspicious ? 'ring-2 ring-red-500/50' : ''}`}>
 {/* Student Row */}
 <button
 ...
 {studentIps.length > 0 && <span className="text-[10px] text-neutral-400">IP: {studentIps.join(', ')}</span>}
 {studentDevices.length > 0 && <span className="text-[10px] text-neutral-400">Thiết bị: {studentDevices.length} máy</span>}
 </div>
 )}
 </div>
 ...
 <span className="text-[10px] text-neutral-600 font-mono shrink-0">{formatTime(v.occurredAt)}</span>
 </div>
 )
 })
 )}
