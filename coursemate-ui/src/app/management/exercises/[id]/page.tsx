...
import {
  ...
  GripVertical, ArrowLeft, Code2, FlaskConical, Info
} from 'lucide-react'
...
export default function ExerciseFormPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const addTestCase = () => setForm(f => ({
    ...f,
    testCases: [...f.testCases, { input: '', expectedOutput: '', description: `Test case ${f.testCases.length + 1}`, isHidden: false, order: f.testCases.length }]
  }))
