...
export const userService = {
  list: async (params?: { filter?: string; pageIndex?: number; pageSize?: number; sorting?: string; role?: string }) => {
  ...
  approveInstructor: (id: string) => api.post<void>(`/api/users/${id}/approve-instructor`),
  toggleLock: (id: string) => api.post<void>(`/api/users/${id}/toggle-lock`)
}
